...
const LanguageSwitcher = ({ translations }: Props) => {
  ...
  return (
    <div className="flex items-center gap-3 font-poppins text-sm text-gray-500">
          ...
          {translations ? (
            ...
            translations[lang.code] ? (
              ...
              <Link
                ...
                className="transition-colors hover:text-[#3d7fdc]"
              >
                {lang.name}
              </Link>
            ) : (
            ...
            // --- 默认模式 (非文章页) ---
            <Link
              href={getLinkForLangDefault(lang.code)}
              className="hover:text-[#3d7fdc]"
            >
              {lang.name}
            </Link>
          )}
